...
const UploadView = ({ onAddProduct }: { onAddProduct: (p: Product) => void }) => {
  ...
  const resizeImage = (base64Str: string, maxWidth = 800): Promise<string> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        // Compress to JPEG 60% which is enough for OCR and saves bandwidth
  ...
  const confirmAdd = async () => {
    if (extractedData) {
      // Ensure we save the optimized version to avoid bloating the sheet
      const thumbToSave = customThumb
        ? await resizeImage(customThumb)
        : (imagePreview ? await resizeImage(imagePreview) : '');
